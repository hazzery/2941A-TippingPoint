...
PID::PID(scalar_t _kP, scalar_t _kI, scalar_t _kD, AbstractMotor::gearset _gearset, string _name) :
...
volt_t PID::Calculate(internal_revolution_t _sensorVal)
{
...
void PID::SetTarget(inch_t _target, millisecond_t _time)
{
    target = _target;
    // std::cout << "Target Has been set to: " << _target << std::endl;

    maxTime = _time;
    startTime = (millisecond_t)pros::millis();
}

//Changes the set point for the PID controler
void PID::SetTarget(inch_t _target)
{
    ...
    millisecond_t time = (_target * 1.5) / ticksPerMilliSecond;
    //time (in ms) = (distance * reality factor) / (ticks per second)

    if (time < 1000_ms) // Seems like a good idea to have some saftey for small moves
        time = 1000_ms; // I choose 1 second at random, so feel free to adjust it

    SetTarget(_target, time);
}

//Gets the target
inch_t PID::GetTarget()
{
    return target;
}